...
#  + matugen integration for Hyprlock color theming
# ============================================================
...
# --- BOOTSTRAP: create fallback colors if file missing -------
if [[ ! -f "$MATUGEN_HYPRLOCK" ]]; then
    cat > "$MATUGEN_HYPRLOCK" << 'EOF'
# Fallback colors — will be replaced by matugen on first wallpaper change
$foreground      = #e7e0e8
...
$error           = #ffb4ab
EOF
    # immediately try to generate real colors from current wallpaper
    if command -v matugen >/dev/null 2>&1; then
        CURRENT_REAL=$(readlink -f "$CURRENT_BG_LINK" 2>/dev/null)
        [[ -f "$CURRENT_REAL" ]] && matugen image "$CURRENT_REAL" --source-color-index 0
    fi
fi
...
# --source-color-index 0 picks dominant color, skips interactive prompt
if command -v matugen >/dev/null 2>&1; then
    matugen image "$NEW_WP" --source-color-index 0
    # pkill -SIGUSR2 waybar   # <-- uncomment if you add matugen colors to waybar later
else
    notify-send "matugen not found" "Install it: yay -S matugen-bin"
fi
